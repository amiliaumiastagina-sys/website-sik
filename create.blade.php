...
@section('content')
<h1 class="text-3xl font-bold mb-8 text-blue-700 tracking-wide">Tambah Poli / Layanan Kesehatan</h1>
    ...
    <form action="{{ route('polis.store') }}" method="POST">
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="kode_poli" value="{{ old('kode_poli') }}" required
            ...
            <div class="col-span-2">
                ...
                <textarea name="deskripsi" rows="3" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('deskripsi') }}</textarea>
            </div>

            <div class="col-span-2">
                <label class="block text-gray-700 font-semibold mb-2">Ruangan</label>
                <input type="text" name="ruangan" value="{{ old('ruangan') }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            ...
            <div class="col-span-2">
                ...
                <select name="doctors[]" multiple class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @foreach($doctors as $doctor)
                        <option value="{{ $doctor->id }}">{{ $doctor->nama_lengkap }} ({{ $doctor->spesialisasi ?? '-' }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-span-2">
                <label class="inline-flex items-center gap-2 mt-2">
                    <input type="checkbox" name="status_aktif" class="form-checkbox h-5 w-5 text-blue-600" checked>
